...
#if 0
        ok = query.exec("insert into books values('0812550757', 'Speaker for the dead', 'New York, N.Y. : TOR, c1986')");
        if ( ! ok)
            qDebug() << "Failed to insert record into table:" << query.lastError();
#endif
    }
#if 0
    QSqlQuery query;
